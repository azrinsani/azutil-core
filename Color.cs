...
namespace AzUtil.Core
{
    public readonly struct Color
	{
		...
		Color(double w, double x, double y, double z, Mode mode)
		{
			...
			switch (mode)
			{
				...
				case Mode.Rgb:
					...
					ConvertToHsl(_r, _g, _b, out _hue, out _saturation, out _luminosity);
					break;
				case Mode.Hsl:
					...
					_a = (float)z.Clamp(0, 1);
					ConvertToRgb(_hue, _saturation, _luminosity,Mode.Hsl, out _r, out _g, out _b);
					break;
			}
		}
		...
		public Color MultiplyAlpha(double alpha)
		{
            return _mode switch
            {
                Mode.Rgb => new Color(_r, _g, _b, _a * alpha, Mode.Rgb),
                Mode.Hsl => new Color(_hue, _saturation, _luminosity, _a * alpha, Mode.Hsl),
                _ => throw new InvalidOperationException("Invalid on Color.Default"),
            };
        }
		...
		public string ToHexSimpleNoHash()
		{
			var red = (uint)(R * 255);
			var green = (uint)(G * 255);
			var blue = (uint)(B * 255);
